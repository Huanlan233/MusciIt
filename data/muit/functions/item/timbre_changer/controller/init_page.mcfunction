...
data remove block 28438403 255 28438400 Items[{Slot:8b}].tag.Muit.timbre_changer_init

data modify block 28438403 255 28438400 Items[{Slot:8b}].tag.pages[0] set value '[{"text":"当前："},{"text":"Minecraft Harp"},"\\n\\n\\n", {"text":"[ 更换音色 ]","clickEvent":{"action":"run_command","value":"/function muit:item/timbre_changer/controller/replace_timbre0 with entity @s Inventory[{Slot:-106b}]"}}]'

data modify storage muit:timbre Root.SelectedTimbre set value {"name":"Minecraft Harp","timbre_id":"minecraft:block.note_block.harp"}
